...
export const chat = async (req, res, next) => {
  try {
    ...
    // Determine whether some course context exists
    ...
    // Keep only recent conversation turns to avoid a huge prompt
    const recentMessages = chatHistory.messages.slice(-6).map(msg => ({
      role: msg.role,
      content: msg.content
    }));
