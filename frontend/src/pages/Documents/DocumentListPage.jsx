...
const DocumentListPage = () => {
  ...
  const handleUpload = async (e) => {
    ...
    if (!uploadFile || !uploadTitle) {
      toast.error("Veuillez fournir un titre et sélectionner un fichier.");
      return;
    }
    setUploading(true);
    ...
    try {
      ...
      toast.error(error.message || "l'importation a échoué.");
    } finally {
      setUploading(false);
    }
  };
            ...
            <form onSubmit={handleUpload} className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="relative border-2 border-dashed border-slate-300 rounded-xl bg-slate-50/50 hover:border-blue-400 hover:bg-blue-50/30 transition-all duration-200">
                  ...
                  <div className="flex flex-col items-center justify-center py-10 px-6">
                    <div className="w-14 h-14 rounded-xl bg-linear-to-r from-blue-100 to-blue-200 flex items-center justify-center mb-4">
                      <Upload
              ...
              <div className="flex gap-3 pt-2">
                <button
                  ...
                  onClick={() => setIsUploadModalOpen(false)}
                  disabled={uploading}
                  className="cursor-pointer flex-1 h-11 px-4 border-2 border-slate-200 rounded-xl bg-white text-slate-700 text-sm font-semibold hover:bg-slate-50 hover:border-slate-300 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  annuler
                </button>
                <button
                  type="submit"
                  disabled={uploading}
                  className="cursor-pointer flex-1 h-11 px-4 bg-linear-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white text-sm font-semibold rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.98]"
                >
                  {uploading ? (
