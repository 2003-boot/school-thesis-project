...
const RegisterPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Compte créé ! Vérifie maintenant ton adresse email.");
      navigate(`/verify-email?email=${encodeURIComponent(email)}`);
    } catch (err) {
      setError(err.error || err.message || "Échec de l’inscription. Veuillez réessayer.");
      toast.error(err.error || err.message || "Échec de l’inscription.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-3xl shadow-xl shadow-slate-200/50 p-10">
          ...
          <div className="text-center mb-10">
            ...
            <p className="text-slate-500 text-sm">
              Commence ton expérience d’apprentissage alimentée par l’IA
            </p>
          </div>
