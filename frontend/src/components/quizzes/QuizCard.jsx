    ...
    <div className="group relative bg-white/80 backdrop-blur-xl border-2 border-slate-200 hover:border-blue-300 rounded-2xl p-4 transition-all duration-200 hover:shadow-lg hover:shadow-emerald-500/10 flex flex-col justify-between">
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          >
            {quiz.title ||
              `Quiz - ${moment(quiz.createdAt).format("D MMM, YYYY")}`}
          </h3>
      ...
      <div className="mt-2 pt-4 border-t border-slate-100">
        {quiz?.userAnswers?.length > 0 ? (
          ...
          <Link to={`/quizzes/${quiz._id}`}>
            <button className="cursor-pointer group/btn relative w-full h-11 bg-linear-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-semibold text-sm rounded-xl transition-all duration-200 shadow-lg shadow-emerald-500/25 active:scale-95 overflow-hidden">
